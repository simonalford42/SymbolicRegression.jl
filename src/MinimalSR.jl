...
function as_policy(policy)
    policy isa Py && (policy = pyconvert(Any, policy))
    policy isa AbstractMinimalSRPolicy && return policy
    sym = Symbol(policy)
    sym in (:default, :minimal, :barebones) && return default_minimal_policy()
    sym in (:pysr, :pysr_compat, :minisr, :compat) && return pysr_compat_policy()
...
function fit_minimal_sr(X_in, y_in, variable_names_in;
    ...
    dominating, n_evals = run_engine(engine, as_policy(policy))
...
function fit_default_sr(args...; kwargs...)
    return fit_minimal_sr(args...; policy=:default, kwargs...)
end

function fit_pysr_compat_sr(args...; kwargs...)
    return fit_minimal_sr(args...; policy=:pysr_compat, kwargs...)
end
